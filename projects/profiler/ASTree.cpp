...
//
void AST::swap(AST& b) {
  nodes nodeTypeTmp = nodeType;
  nodeType = b.nodeType;
  b.nodeType = nodeTypeTmp;
...
//
void AST::mainHeader(std::vector<std::string>& profileName) {
  ...
  for (std::vector<std::string>::iterator name = profileName.begin();
       name != profileName.end(); ++name) {
...
void AST::mainReport(const std::vector<std::string>& profileName) {
  ...
  for(std::list<AST*>::const_iterator mainIT = child.begin(); mainIT != child.end(); ++mainIT) {
    if((*mainIT)->tag == "funtion" && (*mainIT)->getChild("name")->getName() == "main") {
      ...
      for(std::vector<std::string>::const_iterator namesIT = profileName.begin(); namesIT != profileName.end();) {
