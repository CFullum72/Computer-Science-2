...
int main() {
    ...
    srand(9809);  //Seed random number generator.
     ...
     for (int i = 0; i < REP; i++) {
       std::cerr<< i << "loop in REP for loop\n";

       UniqRandInt(MAX, SIZE_OF_SAMPLES, arr);

       for(int j = 0; j < SIZE_OF_SAMPLES; j++) {
        std::cerr<< i << "loop in SIZE_OF_SAMPLES for loop\n";
          std::cout << arr[j] << " ";
       }
       std::cout << std::endl;
     }
...
void UniqRandInt(int max, int n, int result[]) {
  ...
  std::cerr<<"in UniqRandInt\n";
    ...
    while(cntr < n) {
        ...
        r = r % (max + 1);
	std::cerr<< "the random number is" << r;
        if (!(inArray(result, n, r))) {
            result[cntr] =  r;
            cntr++;
        }
    }
...
bool inArray(int array[], int arrSize, int x) {
    for (int i = 0; i < arrSize; ++i) {
        if (array[i] == x) {
	  std::cerr << "r is in the array";
            return true;
        }
    }
    std::cerr << "r is not in the array";
